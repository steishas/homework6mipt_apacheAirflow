# Автоматизация расчетов с помощью Apache AirFlow

**Выполнила**: Смирнова Анастасия

<hr>
 
## 📁 Структура проекта

```text
creating_and_normalazing_db/
├── 📁 python_sripts/           # python-скрипты
│   └── 📄 Смирнова_А.М.py
│── 📁 artifacts                # Скриншоты из Airflow
├── 📄 README.md                # Документация

```

## Описание данных

 <hr>

<details>
 
<summary><b>Таблица 'customer' с полями:</b></summary>

- `сustomer_id` -	ID клиента;
- `first_name` - Имя клиента;
- `last_name` -	Фамилия клиента;
- `gender` -	Пол;
- `DOB` -	Дата рождения;
- `job_title` -	Профессия;
- `job_industry_category` -	Сфера деятельности;
- `wealth_segment` -	Сегмент благосостояния;
- `deceased_indicator` -	Индикатор актуального клиента;
- `owns_car` -	Индикатор наличия автомобиля;
- `address` -	Адрес проживания;
- `postcode` -	Почтовый индекс;
- `state` -	Штаты;
- `country` -	Страна проживания;
- `property_valuation`	Оценка имущества.

</details>

<details>
 
<summary><b>Таблица 'product' с полями:</b></summary>

- `product_id` -	ID продукта;
- `brand` -	Бренд;
- `product_line` -	Линейка продуктов;
- `product_class` -	Класс продукта;
- `product_size` -	Размер продукта;
- `list_price` -	Цена;
- `standard_cost` -	Стандартная стоимость.

</details>

<details>
 
<summary><b>Таблица 'orders' с полями:</b></summary>

- `order_id` -	ID транзакции;
- `customer_id` -	ID клиента;
- `order_date` -	Дата транзакции;
- `online_order` -	Индикатор онлайн-заказа;
- `order_status` -	Статус транзакции.

</details>

<details>
 
<summary><b>Таблица 'order_items' с полями:</b></summary>

- `order_item_id` -	ID позиции в заказе;
- `order_id` -	ID заказа;
- `product_id` -	ID продукта;
- `quantity` -	Количество данного продукта в заказе;
- `item_list_price_at_sale` - Цена продукта в момент продажи;
- `item_standard_cost_at_sale` -	Стандартная стоимость продукта в момент продажи.

</details>

## Структура DAG
 <hr>
 
**Основные этапы обработки***:
- Создание таблиц (4 параллельных таска)

- Загрузка данных из CSV (4 параллельных таска)

- Очистка данных (корректировка таблицы product)

- Выполнение аналитических запросов (2 параллельных таска)

## Технические детали
 <hr>

**Название:** homework_mipt_6_dag

**Расписание:** Ежедневно (@daily)

**Запуск:** С момента создания

**Retry policy:** 1 повторение через 5 минут

**Максимум параллельных тасков:** 10

**EmailOperator заменен на Python-заглушку**

## Используемые операторы:
 <hr>
- `PostgresOperator` - для SQL операций;

- `PythonOperator` - для Python функций;

- `BranchPythonOperator` - для условного выполнения;

- `DummyOperator` - для синхронизации.

## Выполняемые задачи
 <hr>

### 1. Создание таблиц (параллельно):
   
`customer` - данные клиентов;

`product` - данные продуктов;

`orders` - данные заказов;

`order_items` - связь заказов и продуктов.

### 2. Загрузка данных (параллельно):
   
- Автоматическое определение разделителя CSV (, или ;);

- Приведение названий столбцов к нижнему регистру;

- Логирование количества загруженных строк;

### 3. Очистка данных:
   
- Создание таблицы `product_cor` с уникальными `product_id`;

- Удаление дубликатов, сохранение записей с максимальной ценой

### 4. Аналитические запросы:
   
Запрос 1: TOP-3 клиенты по минимальной и максимальной сумме транзакций
```
sql
-- Находит 3 клиентов с минимальной и 3 с максимальной общей суммой транзакций
-- Учитывает клиентов без заказов (сумма = 0)
-- Возвращает: имя, фамилия, общая сумма, тип (минимум/максимум)
```
Запрос 2: TOP-5 клиентов по доходу в каждом сегменте благосостояния

```
sql
-- Для каждого wealth_segment находит 5 клиентов с наибольшим доходом
-- Рассчитывает revenue_generated как сумму покупок
-- Возвращает: имя, фамилия, сегмент, доход, позиция в рейтинге
```
### 5. Проверка результатов:
- Проверка, что оба запроса вернули непустые результаты;

- Если результат пустой → отправка email уведомления.

<details>
<summary><b>Жизненный цикл DAG</b></summary>

<img src="airtifacts/структура dag.png" alt="жизненный цикл dag" width="2000" height="1000">
</details>

## Система уведомлений
- При успехе: Логирование сообщения "DAG выполнен успешно!"

- При ошибке: Логирование ошибки + Отправка email **(заглушка для демо)**

## Дополнительная информация

<details>
<summary><b>Сведения о DAG</b></summary>

<img src="airtifacts/информация о dag.png" alt="информация о dag" width="2000" height="1000">
</details>

<details>
<summary><b>Результат запуска DAG</b></summary>

<img src="airtifacts/выполнение dag.png" alt="выполнение dag" width="2000" height="1000">
</details>

Python-скрипт с DAG приведен [здесь](https://github.com/steishas/homework6mipt_apacheairflow/blob/main/python_scripts/my_dag.py)
